...
class ModsPlugin extends SchemaPlugin {
	...
	function getFieldValue(&$record, $name, $type) {
		...
		switch ($name) {
			...
			case 'url':
				// Simply dump all related item pieces into a
				// single string.
				if (isset($parsedContents['locations'][0]['url']))
					$returner = $parsedContents['locations'][0]['url'];
				break;
		}
	...
	function getTitle(&$record) {
		$value = $this->getFieldValue($record, "title", 0);
		return $value;
	}
	
	function getUrl(&$record) {
		$value = $this->getFieldValue($record, "identifier", 0);
		if(strpos($value, "http") === false) {
			$value = $this->getFieldValue($record, "url", 0);
		}
		return $value; 
	}
