 ...
 * $Id$
 *}
{assign var="pageTitle" value="Advanced Search"}
...
<div class="white">
	<div class="container">
		...
		<form method="post" name="search" action="/misearch/results">
		<input type="hidden" name="isAdvanced" value="1"/>

		<table class="data" width="100%">
		<tr valign="top">
			<td class="text-right"><label for="query">{translate key="search.allFields"} </label></td>
			<td colspan="2" class="value"><input type="text" id="query" name="query" size="40" maxlength="255" value="{$query|escape}" class="textField" /></td>
		</tr>
		<tr valign="top">
			<td class="text-right"><label for="archiveIds">{translate key="archive.archives"} </label></td>
			<td class="value" colspan="2">
				<select onchange="handleArchiveSelect()" multiple id="archiveIds" name="archiveIds[]" size="10" class="selectMenu">
			...
			{foreach from=$crosswalks item=crosswalk}
				...
				{if $crosswalkType == FIELD_TYPE_DATE}
					...
					<tr valign="top">
						<td rowspan="2" class="text-right">{$crosswalk->getName()|escape}</td>
				...
				{elseif $crosswalkType == FIELD_TYPE_SELECT}
					...
					<tr valign="top">
						<td class="text-right"><label for="crosswalk-{$crosswalkId}">{$crosswalk->getName()|escape}</label></td>
						<td colspan="2" class="value">
							<select id="crosswalk-{$crosswalkId}" name="crosswalk-{$crosswalkId}" class="selectMenu">
								<option value="">{translate key="search.select.none"}</option>
								{if $crosswalkId == 5}
								<option value="text">Text</option>
								<option value="image">Images</option>
								<option value="(moving OR video)">Video</option>
								<option value="(sound OR audio)">Sound</option>
			...
			{foreach from=$fields item=field}
				...
				{if $fieldType == FIELD_TYPE_DATE}
					...
					<tr valign="top">
						<td rowspan="2" class="text-right">{$field->getDisplayName()|escape}</td>
						<td class="value">{translate key="common.from"}</td>
				...
				{elseif $fieldType == FIELD_TYPE_SELECT}
					...
					<tr valign="top">
						<td class="text-right"><label for="field-{$fieldId}">{$field->getDisplayName()|escape}</label></td>
						<td colspan="2" class="value">
		...
		<p class="text-center"><input type="submit" value="{translate key="common.search"}" class="btn btn-primary btn-large" /></p>
