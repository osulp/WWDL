...
function selectHarvester() {
	document.getElementById('archiveForm').action="{/literal}{if $archiveId}{url op="editArchive" anchor="archiveForm" path=$archiveId escape=false}{else}{url op="editArchive" anchor="archiveForm" escape=false}{/if}{literal}";
...
<table class="data" width="100%">
	<tr valign="top">
		<td width="20%"  class="text-right">{fieldLabel name="title" key="archive.title" required="true"}</td>
		<td width="80%" class="value"><input type="text" id="title" name="title" value="{$title|escape}" size="40" maxlength="120" class="textField" /></td>
	</tr>
	<tr valign="top">
		<td  class="text-right">{fieldLabel name="description" key="archive.description"}</td>
	...
	<tr valign="top">
		<td  class="text-right">{fieldLabel name="url" key="archive.url" required="true"}</td>
		<td class="value">
	...
	{if $isUserLoggedIn}{* Only administrators are allowed to enter public archive IDs *}
		<tr valign="top">
			<td  class="text-right">{fieldLabel name="url" key="archive.publicArchiveId"}</td>
			<td class="value">
	...
	{if !$isUserLoggedIn}{* Display Captcha test *}
		{if $captchaEnabled}
			<td  class="text-right" valign="top">{fieldLabel name="captcha" required="true" key="common.captchaField"}</td>
			<td class="value">
	...
	<tr>
		<td  class="text-right">{fieldLabel name="harvesterPluginName" key="archive.type" required="true"}</td>
